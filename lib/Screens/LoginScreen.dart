...
import 'package:MuqitTasker/Screens/TaskerScreens/ResetPassword.dart';
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  checkLoginStatus() async {
    ...
    if ((prefs.getString('email')) != null &&
      ...
      if (prefs.getString('userType') == 'Tasker') {
        Navigator.pushReplacement(
            context,
            MaterialPageRoute(
                builder: (context) => MainTaskerScreenSub(
                        ...
                        if (passwordController.text.length > 6) {
                          if (RegExp(
                            ...
                            TaskerLoginModel taskerLoginModel = await TaskerLogin(
                                ...
                                'https://www.muqit.com/app/tasker_login.php');
                            if (taskerLoginModel.status) {
                              // sendAndRetrieveMessage();
                              progressDialog.hide();
                              setLoginSharedPref(
                                  taskerLoginModel.name.trim(),
                                  taskerLoginModel.email.trim(),
                                  taskerLoginModel.id.trim(),
                                  passwordController.text.trim(),
                                  true,
                                  'Tasker');
                              Navigator.pushReplacement(
                              ...
                              progressDialog.hide();
                              Toast.show(taskerLoginModel.message, context,
